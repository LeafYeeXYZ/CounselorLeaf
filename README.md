...
基于以上考虑, 本项目的长时记忆设计为: 让当前上下文中包含"自我概念"、"对用户的概念"、当前上下文 ("短时记忆"), 并让模型按需提取总结后的记忆 ("长时记忆"); 提取的方式是模型自主决策的函数调用 (Function Calling) 🚧. 同时, 短时记忆向长时记忆的更新也由模型通过函数调用来自主决策时机: 当前上下文中会包含每一条消息的时间信息, 模型可以根据时间信息和用户输入来决定是否更新长时记忆; 如果模型发出更新长时记忆的请求, 另一个模型上下文将通过结构化输出的方式更新长时记忆, 并递归更新"自我概念"和"对用户的概念"

🚧 处有三种可能的方案:

1. **在**当前上下文中包含长时记忆的摘要, 并让模型**在需要记忆时**, 通过函数调用来检索指定的长时记忆. 类似 Lee et al. (2024) 的做法, 我开始时最倾向于这种方案, 但其问题是可能导致上下文过于庞大 (也许可以通过长时记忆的"遗忘"来缓解, 但终究会达到极限)
2. **不在**当前上下文中包含长时记忆的摘要, 而是让模型**在需要记忆时**, 通过函数调用, 让**另一个模型**上下文来检索当前模型需要的长时记忆. 类似 MemGPT (Packer et al., 2024) 的做法, 并且后期如果记忆膨胀, 也可以改用 MemoryBank (Zhong et al., 2023) 的方法检索记忆; 此种方法要求"自我概念"和"对用户的概念"能包含足够的信息, 以便减少长时记忆的检索次数; 或者只包含最近 (例如一周) 的长时记忆
3. **不在**当前上下文中包含长时记忆的摘要, 而是**在每次模型推理前**, 让**另一个模型**上下文来检索当前模型需要的长时记忆, 并将其注入当前上下文. 相比 `2`, 可以给用户更多的"惊喜" (即无需显式提示模型记起什么), 但可能会增加计算负载
