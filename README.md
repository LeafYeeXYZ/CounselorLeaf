...
2. **不在**当前上下文中包含长时记忆的摘要, 而是让模型**在需要记忆时**, 通过函数调用, 让**另一个模型**上下文来检索当前模型需要的长时记忆. 类似 MemGPT (Packer et al., 2024) 的做法, 并且后期如果记忆膨胀, 也可以改用 MemoryBank (Zhong et al., 2023) 的方法检索记忆; 此种方法要求"自我概念"和"对用户的概念"能包含足够的信息, 以便减少长时记忆的检索次数
