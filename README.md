...
1. **在**当前上下文中包含长时记忆的摘要, 并让模型**在需要记忆时**, 通过函数调用来检索指定的长时记忆. 类似 Lee et al. (2024) 的做法, 我最倾向于这种方案, 但其问题是可能导致上下文过于庞大 (也许可以通过长时记忆的"遗忘"来解决)
2. **不在**当前上下文中包含长时记忆的摘要, 而是让模型**在需要记忆时**, 通过函数调用, 让**另一个模型**上下文来检索当前模型需要的长时记忆. 类似 MemGPT (Packer et al., 2024) 的做法
