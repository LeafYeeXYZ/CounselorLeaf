  ...
  chatWithMemory: async (chatApi, model, input, vector, plugins) => {
    ...
    if (result.getMemory.count > 0) {
      ...
      if (memories.length > 0) {
        ...
        message = { role: 'assistant', content: '我找遍了我的记忆库, 也没有找到和这个话题相关的记忆', timestamp: Date.now(), memo: true, recall: [] }
      } else {
        message = { role: 'assistant', content: `我此前已经回忆过和"${result.getMemory.description}"相关的记忆, 没有找到新的相似度更高的记忆`, timestamp: Date.now(), memo: true, recall: [] }
      }
