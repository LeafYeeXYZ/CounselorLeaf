  ...
  return (
    <div className='w-full bg-white border border-blue-900 rounded-md px-5 pb-0 pt-4 overflow-auto max-h-full'>
      <Form 
        ...
        layout='vertical' 
      >
        <Form.Item label={<span>嵌入服务地址 <Tag>jina-embedding-v3 Endpoint</Tag></span>}>
          <Space.Compact block>
        ...
        <Form.Item label={<span>嵌入服务密钥 <Tag>jina-embedding-v3 API Key</Tag></span>}>
          <Space.Compact block>
        ...
        <Form.Item label='嵌入向量维度'>
          <Space.Compact block>
            ...
            <Form.Item noStyle name='vectorDimension'>
              <InputNumber style={{ width: '100%' }} onChange={() => setVectorDimensionModified(true)} min={100} max={1500} step={1} />
            </Form.Item>
            <Tooltip title='保存修改' color='blue'>
              <Button
        ...
        <Form.Item label={<span>索引记忆 <Tag>{longTermMemory.filter((item) => item.vector && item.vector.length === vectorDimension).length} / {longTermMemory.length} 已索引</Tag></span>}>
